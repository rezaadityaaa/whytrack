<x-layouts.app>
    <div class="p-6 bg-gradient-to-br from-blue-50 via-white to-blue-100 dark:from-zinc-900 dark:via-zinc-800 dark:to-zinc-900 min-h-screen">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-extrabold text-blue-800 dark:text-blue-200 tracking-tight">Kelola Menu</h2>
            ...
            <table class="min-w-full divide-y divide-blue-100 dark:divide-zinc-700 table-fixed">
                <thead>
                    <tr class="bg-blue-100 dark:bg-zinc-700">
                        ...
                        <th class="py-3 px-4 text-blue-900 dark:text-blue-100 font-semibold text-center">Harga</th>
                ...
                <tbody class="divide-y divide-blue-50 dark:divide-zinc-700">
                    @forelse($menus as $i => $menu)
                        <tr class="hover:bg-blue-50 dark:hover:bg-zinc-700 transition">
                            <td class="py-3 px-4 text-gray-900 dark:text-gray-100 text-center font-medium">{{ $i + 1 }}</td>
                            <td class="py-3 px-4 text-gray-900 dark:text-gray-100 font-medium text-center">{{ $menu->nama }}</td>
                            <td class="py-3 px-4 text-gray-900 dark:text-gray-100 text-center">Rp{{ number_format($menu->harga,0,',','.') }}</td>
                            <td class="py-3 px-4 flex justify-center items-center">
                                ...
                                <form action="{{ route('menu.destroy', $menu->id) }}" method="POST" class="inline ml-2" onsubmit="return confirm('Yakin hapus menu ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-6 text-gray-500 dark:text-gray-400">Belum ada menu.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
